...
export async function updateLineItem(
  ...
  if (!cartId) {
    return formError(toCartErrorMessage('cart-not-found'));
  }
  ...
  if (!line) {
    return formError(toCartErrorMessage('line-item-not-found'));
  }
  ...
  if (line.kind === 'giftCertificate') {
    if (formData.get('remove') === null) {
      return formError(toCartErrorMessage('update-failed'));
    }
    ...
    try {
      ...
      return formError(toSubmissionErrorMessage(error, 'remove-failed'));
    }
  ...
  try {
    ...
    return formError(toSubmissionErrorMessage(error, 'update-failed'));
  }
