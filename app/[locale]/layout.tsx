import type { Metadata } from 'next';
import localFont from 'next/font/local';
import type { ReactNode } from 'react';
import { locale } from 'next/root-params';

import { buildConfig } from '~/lib/config';
import { NextIntlClientProvider } from 'next-intl';
import { getT } from '~/lib/i18n/server';
import { WebVitals } from '~/ui/patterns/web-vitals';
import { getLocales } from '~/data/locales';
import { DEFAULT_LOCALE } from '~/lib/config/channels';

import '~/styles/globals.css';

/*
 * Vendored, not fetched at build time.
 *
 * `next/font/google` self-hosts what it serves, but it still downloads the font
 * from Google *during the build*. A build without egress to fonts.gstatic.com
 * does not fail — it silently ships the fallback stack, so the first anyone
 * notices is that production is rendering in system-ui. That is a bad failure
 * mode for an air-gapped or network-restricted CI, and it makes builds
 * non-reproducible in a way nothing surfaces.
 *
 * The file in ./fonts is the exact woff2 `next/font/google` would have fetched
 * for the latin subset (Inter variable, 48KB). Checked in, so the build has one
 * fewer network dependency and byte-identical output every time.
 */
const inter = localFont({
  // `../fonts` because this layout now sits under the [locale] root param.
  src: '../fonts/inter-latin-variable.woff2',
  // Variable font: one file covers the whole range, so there is no per-weight
  // request and `font-weight` interpolates continuously.
  weight: '100 900',
  variable: '--font-inter',
  display: 'swap',
});

/**
 * `generateMetadata` rather than a static `metadata` export, because the title
 * template is translated and a module-scope `t()` would be evaluated once at
 * import — pinning every locale to whichever one happened to load first.
 */
export async function generateMetadata(): Promise<Metadata> {
  const t = await getT();

  return {
    metadataBase: new URL(buildConfig.get('urls').vanityUrl),
    title: {
      template: `%s | ${t('Common.storeName')}`,
      default: t('Common.storeName'),
    },
  };
}

/**
 * Root layout.
 *
 * Reads NOTHING request-scoped — no `cookies()`, no `headers()`, no
 * `getLocale()`. That is the single most important property of this file.
 *
 * Catalyst's root layout read cookies, which forced every route beneath it to
 * render dynamically and made `generateStaticParams` pointless (its own comment
 * says as much). Everything here comes from build-config or static messages, so
 * the layout lands in the prerendered shell and each route decides its own
 * dynamism.
 *
 * When session-dependent chrome arrives (cart badge, account menu in Phase 1/4),
 * it goes in a `<Suspense>` boundary inside the storefront layout — never here.
 *
 * **It now sits under the `[locale]` root param**, which is what makes locale
 * readable inside `use cache` anywhere in the app without prop drilling. Reading
 * it here is not request-scoped in the sense that matters: a root param is part
 * of the route, so the layout still prerenders — one shell per locale rather
 * than one shell.
 */
export async function generateStaticParams() {
  /*
   * Required, not optional. Under `cacheComponents` a root param with no
   * generated values fails the build outright — there is no "render it on
   * demand" fallback for the segment that defines the shell.
   *
   * Read from BigCommerce rather than a hardcoded list, so adding a language in
   * the control panel is a deploy rather than a code change.
   */
  const locales = await getLocales();

  return locales.map((locale) => ({ locale: locale.code }));
}

export default async function RootLayout({ children }: { children: ReactNode }) {
  const activeLocale = (await locale()) ?? DEFAULT_LOCALE;
  const t = await getT();

  return (
    <html className={inter.variable} lang={activeLocale} suppressHydrationWarning>
      <body>
        <a
          className="sr-only focus:not-sr-only focus:absolute focus:start-4 focus:top-4 focus:z-50 focus:rounded-(--radius-control) focus:bg-primary focus:px-4 focus:py-2 focus:text-primary-foreground"
          href="#main"
        >
          {t('Common.skipToContent')}
        </a>
        {/*
          Client components translate through this provider; Server Components
          use `getT()` directly. Both read the same catalogues, so a string is
          never translated differently depending on which side rendered it.

          Mounted below the skip link so that link is server-rendered markup
          rather than provider children — it must work with no JavaScript.

          No props: rendered from a Server Component, the provider inherits
          locale, messages and timeZone from `i18n/request.ts`. Passing them
          explicitly would duplicate the config and let the two drift.
        */}
        <NextIntlClientProvider>
          {children}
        </NextIntlClientProvider>
        {/* Field performance data, Vercel-only. Renders nothing elsewhere. */}
        <WebVitals />
      </body>
    </html>
  );
}
