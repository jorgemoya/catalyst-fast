import { getFormatCurrency, getFormatDateOnly, getT } from '~/lib/i18n/server';
 ...
 */
export async function LineItemRow({ line }: { line: CartLine }) {
  const t = await getT();
 ...
 */
async function optionValue(option: CartLineOption): Promise<string> {
  const formatDateOnly = await getFormatDateOnly();
 ...
 * after paying.
 */
async function StockNotice({ stock }: { stock: NonNullable<CartLine['stock']> }) {
  const t = await getT();
...
export async function GiftCertificateRow({ line }: { line: CartGiftCertificateLine }) {
  const t = await getT();
  const formatCurrency = await getFormatCurrency();
