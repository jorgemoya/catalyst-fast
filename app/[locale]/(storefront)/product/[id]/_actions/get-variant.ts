...
import { getSelectedCurrencyForAction } from '~/lib/currency';
...
export async function getVariantSnapshot(
  ...
  /*
   * The shopper's currency, not the shell's: this runs on interaction, so it is
   * already dynamic and a cookie read costs nothing. `…ForAction` rather than
   * `getSelectedCurrency` because `next/root-params` is unavailable in Server
   * Actions — see `lib/currency.ts`.
   */
  const currency = await getSelectedCurrencyForAction();

  const [price, availability, inventory] = await Promise.all([
    getProductPrice(productId, currency, optionValueIds),
